...
void hammingServer(string d, bool set)
{
    vector<int> hc;
    for (char c : d) {
    ...
    // Define the distribution for your range (0 to 12)
    ...
    if (set)
    {
        if (hc[random_number]==0) hc[random_number]=1;
        else hc[random_number] =0;
    }
...
int main(int argc, char *argv[])
{
    ...
    while(1)
    {
        ...
        int t; cin >>t;
        if (t==1)
            hammingServer(clientData, true);
        else if (t==2)
            hammingServer(clientData, false);
        else
            exit(0);
